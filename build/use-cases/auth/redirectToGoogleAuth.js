...
const redirectToGoogleAuth = (res, authType = "signup") => {
    const scope = authType === "signup"
        ...
        : ["email"];
    res.redirect(authControllers_1.oauth2Client.generateAuthUrl({ scope, include_granted_scopes: true, state: authType === "signup" ? process.env.GoogleOAuthStateValueForSignup : process.env.GoogleOAuthStateValueForLogin }));
};
